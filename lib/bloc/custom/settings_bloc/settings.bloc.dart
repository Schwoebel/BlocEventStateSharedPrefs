...
class SettingsBloc
  extends BlocEventStateBase<SettingsEvent, SettingsState> {
  SettingsBloc() {
    _loadAppDefaults();
  ...
  Future<void> _loadAppDefaults() async {
    ...
    defaults.forEach(
        ...
        final _sharedPreference = sharedPrefs.get(key.toString());
  ...
  Future<void> _loadSharedPreferences() async {
    final sharedPrefs = await SharedPreferences.getInstance();
    final _ttsPitchPref =
    sharedPrefs.getDouble(SettingsKeys.text_to_speech_pitch.toString());
    final _ttsRatePref =
    sharedPrefs.getDouble(SettingsKeys.text_to_speech_rate.toString());
    final _appLanguagePref =
    sharedPrefs.getString(SettingsKeys.app_language.toString());
    emitEvent(InitializedSettingsEvent(
      _ttsPitchPref, _ttsRatePref, _appLanguagePref));
  }
